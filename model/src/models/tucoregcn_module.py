...
if __name__ == "__main__":
    ...
    cfg = omegaconf.OmegaConf.load(root / "configs" / "model" / "tucoregcn.yaml")
    model = hydra.utils.instantiate(cfg)
    dfg = omegaconf.OmegaConf.load(root / "configs" / "datamodule" / "tucoregcn_datamodule.yaml")
    datamodule = hydra.utils.instantiate(dfg)
    datamodule.setup()
    data = next(iter(datamodule.train_dataloader()))
    input_ids = data["input_ids"]
    token_type_ids = data["token_type_ids"]
    attention_mask = data["input_masks"]
    speaker_ids = data["speaker_ids"]
    label_ids = data["label_ids"]
    mention_ids = data["mention_ids"]
    turn_mask = data["turn_masks"]
    graphs = data["graphs"]
