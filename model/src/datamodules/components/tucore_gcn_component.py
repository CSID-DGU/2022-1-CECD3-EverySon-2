...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, token_type_ids, label_id, speaker_ids, mention_ids):
...
def tokenize(text, tokenizer, start_mention_id):
    speaker2id = {'[unused1]': 11, '[unused2]': 12, 'speaker 1': 1, 'speaker 2': 2, 'speaker 3': 3, 'speaker 4': 4,
                  'speaker 5': 5, 'speaker 6': 6, 'speaker 7': 7, 'speaker 8': 8, 'speaker 9': 9}
    D = ['[unused1]', '[unused2]', 'speaker 1', 'speaker 2', 'speaker 3', 'speaker 4', 'speaker 5', 'speaker 6',
    ...
    speaker_id = 0
    for t in textraw:
        if t in ['speaker 1', 'speaker 2', 'speaker 3', 'speaker 4', 'speaker 5', 'speaker 6', 'speaker 7', 'speaker 8',
                 'speaker 9']:
...
def convert_examples_to_features(examples, max_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        _truncate_seq_tuple(tokens_a, tokens_b, tokens_c, max_length - 4, tokens_a_speaker_ids,
        ...
        assert len(speaker_ids) == max_length
        assert len(mention_ids) == max_length
...
def convert_examples_to_features_roberta(examples, max_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        _truncate_seq_tuple(tokens_a, tokens_b, tokens_c, max_length - 6, tokens_a_speaker_ids,
                            tokens_b_speaker_ids, tokens_c_speaker_ids, tokens_a_mention_ids)
        tokens_b_mention_ids = [max(tokens_a_mention_ids) + 1 for _ in range(len(tokens_b))]
        tokens_c_mention_ids = [max(tokens_a_mention_ids) + 2 for _ in range(len(tokens_c))]
        ...
        assert len(token_type_ids) == len(speaker_ids)
...
def _truncate_seq_tuple(tokens_a, tokens_b, tokens_c, max_length, tokens_a_speaker_ids, tokens_b_speaker_ids,
                        tokens_c_speaker_ids, tokens_a_mention_ids):
...
def mention2mask(mention_id, window_size=1):
    slen = len(mention_id)
    mask = []
    ...
    for j in range(slen):
        ...
        if mention_id[j] not in turn_mention_ids:
            ...
            if mention_id[j] - window_size in turn_mention_ids:
                start = mention_id[j] - window_size

            if mention_id[j] + window_size in turn_mention_ids:
                end = mention_id[j] + window_size
...
class TUCOREGCNDataset(IterableDataset):
    ...
    def __init__(self, src_file, save_file, max_length, tokenizer, n_class, encoder_type, window_size=1):

        super(TUCOREGCNDataset, self).__init__()

        self.data = None
        self.input_max_length = max_length
        ...
        if os.path.exists(save_file):
            ...
            if "train" in save_file:
                ...
                raise ValueError(f"{save_file} is not expected.")

            if encoder_type == "BERT":
                features = convert_examples_to_features(examples, max_length, tokenizer)
            else:
                features = convert_examples_to_features_roberta(examples, max_length, tokenizer)
            ...
            for f in features:
                ...
                self.data.append({
                    ...
                    'turn_mask': mention2mask(np.array(f[0].mention_ids), window_size),
                    'graph': graph
                })
            # save data
...
class TUCOREGCNDataset4f1c(IterableDataset):
    ...
    def __init__(self, src_file, save_file, max_length, tokenizer, n_class, encoder_type):
...
class TUCOREGCNDataloader(DataLoader):
    ...
    def __iter__(self):
        ...
        # begin
        input_ids = torch.LongTensor(self.batch_size, self.max_length)
        token_type_ids = torch.LongTensor(self.batch_size, self.max_length)
        input_mask = torch.LongTensor(self.batch_size, self.max_length)
        mention_id = torch.LongTensor(self.batch_size, self.max_length)
        speaker_id = torch.LongTensor(self.batch_size, self.max_length)
        turn_masks = torch.LongTensor(self.batch_size, self.max_length, self.max_length)
        label_ids = torch.Tensor(self.batch_size, self.relation_num)

        for idx, minibatch in enumerate(self.batches):
            cur_bsz = len(minibatch)

            for mapping in [input_ids, token_type_ids, input_mask, mention_id, label_ids, turn_masks, speaker_id]:
            ...
            for i, example in enumerate(minibatch):
                mini_input_ids, mini_token_type_ids, mini_input_mask, mini_label_ids, mini_mention_id, mini_speaker_id, turn_mask, graph = \
                    example['input_ids'], example['token_type_ids'], example['input_mask'], example['label_ids'], \
                ...
                token_type_ids[i, :word_num].copy_(torch.from_numpy(mini_token_type_ids))
