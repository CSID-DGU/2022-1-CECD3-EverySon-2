...
def predict(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    save_result(
        ...
        predictions=preds,
        class_names=cfg.datamodule.class_names
    )
...
def save_result(data_dir, output_dir, predictions, class_names):
    import pandas as pd
    import os.path

    idx2label = {k: v for k, v in enumerate(class_names)}

    df = pd.read_csv(data_dir, encoding="UTF-8")
    df["label"] = predictions
    df["label"] = df["label"].apply(lambda x: idx2label[x])
